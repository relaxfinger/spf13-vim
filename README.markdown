...
```bash
    ...
    curl https://url.cn/5m9opnY -L > spf13-vim.sh && sh spf13-vim.sh
```

If you have a bash-compatible shell you can run the script directly:
```bash

    sh <(curl https://url.cn/5m9opnY -L)
```
...
Add the UnPlugin command to this line. It takes the same input as the Plugin line, so simply copy the line you want to disable and add 'Un' to the beginning.

For example, disabling the 'AutoClose' and 'scrooloose/syntastic' plugins

```bash
    echo UnPlugin \'AutoClose\' >> ~/.vimrc.bundles.local
    echo UnPlugin \'scrooloose/syntastic\' >> ~/.vimrc.bundles.local
```
